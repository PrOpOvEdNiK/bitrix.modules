...
class phpthumb {
	...
	function setSourceImageResource($gdimg) {
        ...
        var_export('setSourceImageResource');
		return true;
	}
...
//		}
	...
	function ResolveFilenameToAbsolute($filename) {
		...
		if (preg_match('#^[a-z0-9]+\:/{1,2}#i', $filename) || preg_match('#^[a-z]:\\\\{1}#iu', $filename)) {
...
// newer versions of ImageMagick require -repage <geometry>
				...
				if (!@file_exists($IMtempfilename) || !@filesize($IMtempfilename)) {
					...
					} elseif (function_exists(@$ImageCreateFunction) && ($this->gdimg_source = @$ImageCreateFunction($IMtempfilename) && (var_export('ImageCreateFunction')))) {
	...
	function ExtractEXIFgetImageSize() {
		...
		if ($this->config_use_exif_thumbnail_for_speed && $this->exif_thumbnail_data) {
			while (true) {
                ...
                var_export('ImageCreateFunction2');
