...
class Zend_Db_Profiler
{
    ...
    public function fillBitrixQuery() {
        global $DB;
        ...
        if (!empty($queries)) {
            foreach($queries as $query) {
                $q = self::trimBitrixSql($query);

                $id = $this->queryStart(self::trimBitrixSql($query['QUERY']), null, true);
                $this->queryEnd($id);
                $this->_queryProfiles[$id]->bitrixFixElapsed($query['TIME']);
            }
    ...
    public static function trimBitrixSql($query) {

        $list = explode("\n", $query);
        $rList = array();
        foreach ($list as $index => $str){
            $list[$index] = str_replace("\r", "", trim($str));
            if (!empty($list[$index])) {
                $rList[] = $list[$index];
            }
        }
     ...
     * @return integer|null
     */
    public function queryStart($queryText, $queryType = null, $bitrix = false)
    {
        if (!$this->_enabled) {
            return null;
        }
     ...
     *
     * @return Zend_Db_Profiler_Query|bool
     */
