...
class CTR
{
	...
	{
		$t             = microtime(true);
		$this->counter = [(int)$t & 0xFFFFFFFF, (($t - (int)$t) * 0x100000000) & 0xFFFFFFFF, 0, 0];
	...
	private function set_nonce($nonce)
	{
		...
		$this->counter = [$this->counter[1], $this->counter[2], 0, 0];
	}

	public function inc()
	{
		return self::ninc($this->counter[3]) || self::ninc($this->counter[2]) || self::ninc($this->counter[1]);
	}
