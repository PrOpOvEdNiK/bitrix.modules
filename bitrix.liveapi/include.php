...
/*
	...
	static function OnAdminPageLoad()
	{
	}

	static function ScanModule($path0)
	{
		$arRes = array();
		...
		while(false !== ($l = fgets($f)))
		{
			...
			if (!$js)
			{
				...
				elseif (preg_match('#^([a-z\s]*)function (&?[a-z0-9_]+) ?\(((.*)\))?#i', $l, $regs))
				{
					if ($interface)
						continue;
					if (preg_match('#\babstract\b#i', $regs[1]))
						continue;
				...
				elseif (preg_match('#(\\\\Bitrix\\\\Main\\\\Event|GetModuleEvents|CLearnHelper::FireEvent)\([^,]+,["\' ]*([a-z0-9_]+)#i', $l, $regs))
				{
					$event = $regs[2];
	...
	static function ResMerge($ar0, $ar1)
	{
		$arRes = array();
	...
	static function Beautiful($html, $name, $namespace, $file, $line, $highlight)
	{
		global $raw;
		$raw = $html;
		$html = "<?"."php \n//	".$namespace.$name."()\n//	$file:$line\n\n".$html;
	...
	static function colorize($func, $ar, $class = false, $module_id = '')
	{
	...
	static function ReadOld()
	{
		include(DATA_FILE_OLD);
		return $DATA;
	}

	static function Insert($arRawFields)
	{
		global $DB;
	...
	static function PrepareFields($arRawFields)
	{
		global $DB;
		$arFields = array();
	...
	static function Clear($arFilter = array())
	{
		global $DB;
		if (!count($arFilter))
	...
	static function GetModuleVersion($module_path, $module_id)
	{
		if ($module_id == 'main')
		{
